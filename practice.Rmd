...
pd<-expand.grid(x=seq(-4,4,.04),y=seq(-3,12,.04))
...
(base_plot<-ggplot(pd)+geom_contour(aes(x=x,y=y,z=log1p(z),colour=..level..))+scale_colour_continuous(low="red",high="blue")+geom_point(data=solution,aes(x,y),colour="red",size=10,shape="x")+theme_bw())
ggs("rosenbrock")
```
...
nloptr_lbfgs<-function(f,g,x_init,maxIter=MAXITER){
  opts<-list(algorithm="NLOPT_LD_LBFGS",ftol_abs=TOLERANCE)
  nloptr(x0=x_init,eval_f=function(x){f(x)},eval_grad_f=function(x){g(x)},opts=opts)
}
